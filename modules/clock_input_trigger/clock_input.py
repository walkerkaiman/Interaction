        ...
        while self._running:
            ...
            if self.target_time == now and self._last_fired != now:
                ...
                if getattr(self, 'log_level', 'info') == 'verbose':
                    self.log_callback(f"📤 Clock: Sending event to {len(self._event_callbacks)} callbacks")
                for cb in list(self._event_callbacks):
                    try:
                        if getattr(self, 'log_level', 'info') == 'verbose':
                            self.log_callback(f"📤 Clock: Calling callback: {cb}")
                        cb(event)
                        if getattr(self, 'log_level', 'info') == 'verbose':
                            self.log_callback(f"✅ Clock: Event sent successfully")
                    except Exception as e:
                        self.log_callback(f"❌ Error in clock event callback: {e}")
