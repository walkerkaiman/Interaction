...
const Performance: React.FC = () => {
  const STATS_STORAGE_KEY = 'performanceStats';
  const MEM_STORAGE_KEY = 'performanceMemoryStats';
  ...
  const [memoryStats, setMemoryStats] = useState<MemoryPoint[]>(() => {
    const stored = localStorage.getItem(MEM_STORAGE_KEY);
    if (stored) {
      try { return JSON.parse(stored); } catch { return []; }
    }
    return [];
  ...
  useEffect(() => {
    localStorage.setItem(MEM_STORAGE_KEY, JSON.stringify(memoryStats));
  }, [memoryStats]);
